...
public class ProductDescriptionActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        for (int i = 0; i < ProductList.size(); i++) {
            if (ProductList.get(i).getProduct_id().equals(Model.ProductId)) {
                desc = ProductList.get(i).getDescription();
                img_url = ProductList.get(i).getMain_image();
                desc_heading = ProductList.get(i).getProduct_title();
                model = ProductList.get(i).getModel();
            }
        }
