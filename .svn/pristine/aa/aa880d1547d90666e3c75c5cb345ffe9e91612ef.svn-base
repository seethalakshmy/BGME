...
public class MsdsActivity extends AppCompatActivity {
    private ArrayList<Document> MsdsList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (Model.Language.equals( "en" ))
            ...
            mHead.setText( getString( R.string.msds_ar ) );
        ...
        MsdsList = new ArrayList<>();

        if (Parser.DocumentList.size() != 0) {
            for (int i = 0; i < Parser.DocumentList.size(); i++) {
                if (Parser.DocumentList.get(i).getType().equals("MSDS"))
                    MsdsList.add(Parser.DocumentList.get(i));
            }
            if(MsdsList.size()==0) {
            ...
            adapter = new GridItemAdapter(MsdsList, this);
            recyclerView.setAdapter(adapter);

            adapter.SetOnItemClickListener(new GridItemAdapter.OnItemClickListener() {
                @Override
                public void onItemClick(View view, int position) {
                    model.DocURL = MsdsList.get(position).getRecord();
                    Intent in = new Intent(MsdsActivity.this, DocViewActivity.class);
                    startActivity(in);
