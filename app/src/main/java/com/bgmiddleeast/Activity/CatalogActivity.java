...
public class CatalogActivity extends AppCompatActivity {
    private  ArrayList<Document> CatalogList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (Model.Language.equals( "en" ))
            mHead.setText("Catalog");
        else
            mHead.setText( getString( R.string.calatog_ar ) );

        CatalogList = new ArrayList<>();

        if(Parser.DocumentList.size()!=0) {
            for (int i=0;i<Parser.DocumentList.size();i++){
                if(Parser.DocumentList.get(i).getType().equals("Catalog"))
                    CatalogList.add(Parser.DocumentList.get(i));
            }
            ...
            adapter = new GridItemAdapter(CatalogList, this);
            recyclerView.setAdapter(adapter);
            adapter.SetOnItemClickListener(new GridItemAdapter.OnItemClickListener() {
                @Override
                public void onItemClick(View view, int position) {
                    model.DocURL = CatalogList.get(position).getRecord();
                    Intent in = new Intent(CatalogActivity.this, DocViewActivity.class);
                    startActivity(in);
